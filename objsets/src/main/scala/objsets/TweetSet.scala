...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    if (!left.isEmpty && p(elem))
      left.filterAcc(p, acc.incl(elem))
    else if (!right.isEmpty && p(elem))
  ...
  def descendingByRetweet: TweetList = {
    ...
    //    def sort(set: TweetSet, acc: TweetList) : TweetList = {
    //      if(!set.isEmpty)
    //         sort(set.remove(set.mostRetweeted), new Cons(set.mostRetweeted, acc))
    //      else acc
    //    }
    //
    //    sort(this, Nil)
    ...
    def sort(set: TweetSet, acc: TweetList): TweetList = {
      if (set.isEmpty) new Cons(mostRetweeted, acc)
      else sort(set.remove(set.mostRetweeted), acc)
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => (google.exists((s:String) => { println(s + " " + tw.text); tw.text.contains(s)})))

  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tw => apple.exists((s:String) => tw.text.contains(s)))
